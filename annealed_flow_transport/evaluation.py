...
class ExpectationLogger(object):
    ...
    def __init__(self, expectation_config, num_dim: int):
        ...
        self._averages = []
        for config, label in zip(self._expectation_config.configurations, self._labels):
    ...
    def record_expectations(self, particle_state: ParticleState):
        ...
        for index, expectation in enumerate(self._expectations):
            expectation_val = expectation(
                particle_state.samples, particle_state.log_weights
            )
...
def get_particle_propose(config) -> ParticlePropose:
    ...
    if is_annealing_markov_kernel_algorithm(config.algo):
        density_by_step = flow_transport.GeometricAnnealingSchedule(
            log_density_initial, log_density_final, config.base_steps * config.steps_mult + 1
        )
        markov_kernel_by_step = markov_kernel.MarkovTransitionKernel(
            config.mcmc_config, density_by_step, config.base_steps * config.steps_mult + 1
        )
    ...
    if config.algo == "smc":
        ...
        def particle_propose(loc_key: RandomKey):
            return smc.fast_outer_loop_smc(
                density_by_step, initial_sampler, markov_kernel_by_step, loc_key, config
            )
        ...
        @jax.jit
        def particle_propose(loc_key: RandomKey):
            return craft.craft_evaluation_loop(
                loc_key,
                flow_params,
        ...
        @jax.jit
        def particle_propose(loc_key: RandomKey):
            return vi.vfe_naive_importance(
                initial_sampler,
                log_density_initial,
                log_density_final,
                flow_forward_fn.apply,
...
def get_expectation_logger(expectation_config, num_dim: int) -> ExpectationLogger:
...
def run_experiment(config):
    ...
    expectation_logger = get_expectation_logger(
        config.expectation_config, (config.num_dims,)[0]
    )
