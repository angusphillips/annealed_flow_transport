...
def inner_loop_snf(
    ...
    initial_samples = initial_sampler(subkey, config.snf_batch_size, (config.num_dims,))
...
def outer_loop_snf(
    ...
    def vi_update(curr_key, curr_transition_params, curr_opt_state):
        ...
        updates, new_opt_state = opt.update(flow_grads, curr_opt_state)
        new_transition_params = optax.apply_updates(curr_transition_params, updates)
    ...
    place_holder_array = jnp.array([0.0])
        ...
        if step % config.report_step == 0:
            if log_step_output is not None:
                ...
                log_step_output(
                    ...
                    log_normalizer_estimate=-1.0 * vfe,
    ...
    final_log_normalizer_estimate = -1.0 * vfe
