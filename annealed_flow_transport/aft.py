...
def get_initial_samples_log_weight_tuples(
    initial_sampler: InitialSampler, key: RandomKey, config
    ...
    samples_tuple = SamplesTuple(
        *[
            initial_sampler(elem, batch, (config.num_dims,))
...
def outer_loop_aft(
    ...
    if log_step_output is not None:
        zero_vfe_tuple = VfesTuple(
            train_vfes=jnp.zeros(opt_iters), validation_vfes=jnp.zeros(opt_iters)
        )
        log_step_output(samples_tuple, log_weights_tuple, zero_vfe_tuple, 0.0, 1.0, 1.0)
