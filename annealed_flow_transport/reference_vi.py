...
from annealed_flow_transport.distributions import MeanFieldNormalDistribution
...
def get_variational_approx(cfg, rng, target_distribution):
    ...
    def variational_log_density(x: Array) -> Array:
        vld = VariationalLogDensity(dim=cfg.num_dims, dtype=jnp.float32, name="Variational")
        return vld(x)

    def variational_sampler(sampler_rng: Key, num_particles: int) -> Array:
        v_sampler = VariationalSampler(
            dim=cfg.num_dims, dtype=jnp.float32, name="Variational"
        )
    ...
    samples = jnp.ones((cfg.vi_optim.batch_size, cfg.num_dims))
