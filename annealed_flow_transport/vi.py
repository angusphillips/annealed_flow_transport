...
def vfe_naive_importance(
    ...
    samples = initial_sampler(key, config.batch_size, (config.num_dims,))
...
def outer_loop_vi(
    ...
    def vi_free_energy_short(loc_flow_params, loc_key):
        return vi_free_energy(
            loc_flow_params,
            loc_key,
            initial_sampler,
            initial_log_density,
    ...
    while step < config.vi_iters:
        with jax.profiler.StepTraceAnnotation("train", step_num=step):
            key, flow_params, curr_free_energy, opt_state = jit_vi_update(
            ...
            if step % config.vi_report_step == 0:
                ...
                if config.vi_estimator == "elbo":
                    log_normalizer_estimate = -1.0 * curr_free_energy
                elif config.vi_estimator == "importance":
                    subkey, key = jax.random.split(key, 2)
                    log_normalizer_estimate = jit_vfe_naive_importance(
                        flow_params, subkey
                    )
                else:
    ...
    results = AlgoResultsTuple(
        ...
        delta_time=0.0,  # These are currently set with placeholders.
